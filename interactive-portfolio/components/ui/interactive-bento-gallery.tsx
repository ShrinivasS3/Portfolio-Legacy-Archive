      ...
      : items.filter((item) => {
          if (activeFilter === "Art")
            return (
              ...
              item.title.toLowerCase().includes("abstract")
            )
          if (activeFilter === "Photography")
            return (
              item.title.toLowerCase().includes("photography") ||
              item.title.toLowerCase().includes("portrait") ||
              item.title.toLowerCase().includes("landscape") ||
              item.title.toLowerCase().includes("urban")
            )
          if (activeFilter === "3D")
            return (
              item.title.toLowerCase().includes("3d") ||
              item.title.toLowerCase().includes("character") ||
              item.title.toLowerCase().includes("architectural")
            )
          return true
        })
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-4xl">
      <div className="mb-8 text-center">
        ...
        >
          {["All", "Art", "Photography", "3D"].map((filter) => (
            <button
              key={filter}
              ...
              className={`px-6 py-2 rounded-full transition-colors duration-300 ${
                activeFilter === filter
                  ...
                  : "bg-gray-700 text-gray-300 hover:bg-[#3867D6] hover:text-white"
              }`}
            >
              {filter}
            </button>
