...
const AnimatedFooter: React.FC<FooterProps> = ({ leftLinks, rightLinks, copyrightText, barCount = 23 }) => {
  ...
  return (
    <footer
      ref={footerRef}
      className="bg-gray-900 text-white relative flex flex-col w-full h-full justify-between select-none"
    >
        ...
        <div className="space-y-2">
          <ul className="flex flex-wrap gap-4">
            {leftLinks.map((link, index) => (
              <li key={index}>
                <a href={link.href} className="text-sm hover:text-[#3867D6] transition-colors">
                  {link.label}
                </a>
              </li>
            ))}
        ...
        <div className="space-y-4">
          <ul className="flex flex-wrap gap-4">
            {rightLinks.map((link, index) => (
              <li key={index}>
                <a href={link.href} className="text-sm hover:text-[#F5A623] transition-colors">
                  {link.label}
                </a>
              </li>
            ))}
          ...
          <div className="text-right mt-4">
            <button
              onClick={scrollToTop}
              className="text-sm hover:text-[#20C997] hover:underline inline-flex items-center transition-colors"
            >
              Back to top
            </button>
          </div>
        ...
        <div style={{ marginTop: 0 }}>
          {Array.from({ length: barCount }).map((_, index) => (
            <div
              ...
              style={{
                height: `${index + 1}px`,
                backgroundColor: index % 3 === 0 ? "#3867D6" : index % 3 === 1 ? "#F5A623" : "#20C997",
